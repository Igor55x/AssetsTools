...
namespace AssetsTools
{
    public class AssetsFileInstance
    {
        ...
        public AssetsFileInstance(FileReader reader, string root)
        {
        ...
        public AssetsFileInstance GetDependency(AssetsManager am, int depIdx)
        {
            ...
            var instIndex = am.LoadedFiles.FindIndex(f => string.Equals(Path.GetFileName(f.path), Path.GetFileName(depPath), StringComparison.CurrentCultureIgnoreCase));
            if (instIndex == -1)
            {
            ...
            {
                dependencies[depIdx] = am.LoadedFiles[instIndex];
            }
            return dependencies[depIdx];
        }
    }
